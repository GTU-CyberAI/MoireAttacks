...
def back_to_rotation(org_images, theta, phi, gamma,):

    rotate_images = [np.array(np.zeros(org_images.shape))]
    rotate_images = np.array(rotate_images)

    org_images_ = [org_images]
    org_images_ = np.array(org_images_)

    for n, img in enumerate(org_images_):
        Trans_org = ImageTransformer(img)

        rtheta, rphi, rgamma = Trans_org.get_rad(-theta, -phi, -gamma)

        # Get ideal focal length on z axis
        # NOTE: Change this section to other axis if needed
        d = np.sqrt(Trans_org.height ** 2 + Trans_org.width ** 2)
        Trans_org.focal = d / (2 * np.sin(rgamma) if np.sin(rgamma) != 0 else 1)
        dz = Trans_org.focal

        # Get projection matrix
        mat = Trans_org.get_M(rtheta, rphi, rgamma, 0, 0, dz)
        # roated_img = cv2.warpPerspective(self.image.copy(), mat, (self.width, self.height))
        temp = np.array(Trans_org.image)
        rotate_img = cv2.warpPerspective(temp, mat, (Trans_org.width, Trans_org.height))
    ...
    return rotate_images[0]

def check_valid(image, labels):

    h, w = image.shape[:2]
    for label in labels:
        class_id, x_center, y_center, box_w, box_h = map(float, label[:5])
        ...
        if len(crop.shape) == 3:
            black_pixels = np.all(crop == 0, axis=-1)
        else:  # grayscale
            black_pixels = crop == 0

        black_ratio = np.sum(black_pixels) / crop.size
        #print(black_ratio)
        if black_ratio >= 0.10:
            return False
...
def read_yolo_labels(txt_path):
    ...
    with open(txt_path, "r") as f:
        for line in f:
            ...
            if len(parts) >= 5:
                labels.append(parts[:5])  # Take only first 5 floats (class_id, x_center, y_center, w, h)
    return labels
...
class Morie_attack(Attack):
    ...
    def random_rotation_3(self, org_images, lcd_images):
        ...
        return rotate_images, rotate_lcd_images, theta, phi, gamma
    ...
    def forward(self, org_imgs, org_labels, targeted_labels):
        ...
        rotate_images, rotate_lcd_images, theta, phi, gamma = self.random_rotation_3(temp_images, lcd_images)
        ...
        for step in range(self.steps):
            ...
            at_images = F.interpolate(brighter_img, [800, 800], mode='bilinear')
        ...
        return at_images, rotate_images, dim_images, \
               ...
               org_percentage, at_percentage, rotate_percentage, dim_percentage, theta, phi, gamma
...
if __name__ == "__main__":
    ...
    normal_data = image_folder_custom_label(root='/selected_humans/images', transform=transform,
    ...
    txt_path = "/selected_humans/labels"
    for txt_fie in sorted(os.listdir(txt_path)):
        txt_at_name = txt_fie[:-4] + "_" + str(0) + ".txt"
        txt_at_path = os.path.join(dest_path + "labels", txt_at_name)
        if os.path.exists(txt_at_path):
            org_imgs, org_labels = next(normal_iter)
            print(f"{txt_at_path} passed.")
            continue
        print(txt_fie)
        for batch in range(epoch):

            print("-" * 70)
            org_imgs, org_labels =next(normal_iter) #normal_iter.next()
            org_imgs = org_imgs * 255.0
            print('Epoch = ' + str(batch))

            targeted_labels = np.random.randint(0,1)
            #num_classes = len(idx2label)
            #targeted_labels = np.random.randint(0, num_classes)
            targeted_labels = torch.from_numpy(targeted_labels * np.ones((batch_size), dtype = int))
            targeted = True

            steps = random.randint(30, 50)
            print("steps = ", steps)
            attack = Morie_attack(model, noise_budget = noise_budget, img_w = 400, img_h = 400, scale_factor = 3, targeted = targeted, batch_size=batch_size, steps = steps)
            at_images, rotate_images, dim_images, \
            at_labels, rotate_labels, dim_labels, \
            org_percentges, at_percentages, rotate_percentages, dim_percentages, theta, phi, gamma = attack(org_imgs, org_labels, targeted_labels)
...
#
            ...
            for i in range(batch_size):
                ...
                if Save_results == 'True':
                    ...
                    labels = read_yolo_labels(os.path.join(txt_path, txt_fie))
                    img_at_np = np.array(back_to_rotation(np.array(img_at, dtype = (np.uint8)), theta, phi, gamma), dtype = (np.uint8))
                    img_at_bgr = cv2.cvtColor(img_at_np, cv2.COLOR_RGB2BGR)
                    if check_valid(img_at_bgr, labels):
                        total_img_ctr += 1

                        img_at_name = txt_fie[:-4] + "_" +str(0) + ".JPEG"
                        img_at_path = os.path.join(dest_path + "images", img_at_name)
                        txt_at_name = txt_fie[:-4] +  "_" + str(0) + ".txt"
                        txt_at_path = os.path.join(dest_path + "labels", txt_at_name)

                        cv2.imwrite(img_at_path, img_at_bgr)
                        shutil.copy(os.path.join(txt_path, txt_fie), txt_at_path)
        if total_img_ctr >= 2000:
            break
